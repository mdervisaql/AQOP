...
class AQOP_Leads_Admin
{
	...
	public function __construct()
	{
		...
		add_action('wp_ajax_aqop_add_campaign_question', array($this, 'ajax_add_campaign_question'));
		add_action('wp_ajax_aqop_edit_campaign_question', array($this, 'ajax_edit_campaign_question'));
		add_action('wp_ajax_aqop_delete_campaign_question', array($this, 'ajax_delete_campaign_question'));

		// Meta Integration AJAX handlers
		add_action('wp_ajax_aqop_get_meta_test_payload', array($this, 'ajax_get_meta_test_payload'));
		add_action('wp_ajax_aqop_sync_airtable', array($this, 'sync_airtable'));
		add_action('wp_ajax_aqop_test_sync_airtable', array($this, 'test_sync_airtable'));
		add_action('wp_ajax_aqop_sync_airtable_chunk', array($this, 'sync_airtable_chunk'));
		add_action('wp_ajax_aqop_fetch_airtable_fields', array($this, 'fetch_airtable_fields_ajax'));
		add_action('wp_ajax_aqop_test_whatsapp_connection', array($this, 'ajax_test_whatsapp_connection'));
		add_action('wp_ajax_aqop_save_auto_sync_settings', array($this, 'ajax_save_auto_sync_settings'));
	}
	...
	{
		// Check if user has permission (any AQOP role).
		if (!$this->user_has_aqop_access()) {
			return;
		}
		...
		// === ACTIVITY MONITOR (Phase 6) ===
		add_submenu_page(
			'aqop-control-center',
			__('Activity Monitor', 'aqop-leads'),
			__('Activity Monitor', 'aqop-leads'),
			'manage_options',
			'aqop-activity-monitor',
	...
	public function render_leads_page()
	{
		...
		<div class="wrap aqop-leads-admin">
			...
			<a href="<?php echo esc_url(add_query_arg(array('page' => 'aqop-leads-form'), admin_url('admin.php'))); ?>"
	...
	{
		include AQOP_LEADS_PLUGIN_DIR . 'admin/views/activity-monitor.php';
	}
	...
	public function handle_settings_save()
	{
		...
		switch ($action) {
			...
			case 'update_notifications':
				...
				break;
			case 'update_cors':
				$this->update_cors_settings();
				break;
			case 'update_meta_integration':
				$this->update_meta_integration_settings();
				break;
			case 'update_whatsapp_settings':
				$this->update_whatsapp_settings();
				break;
			case 'add_country':
				$this->add_country();
				break;
			case 'add_campaign_group':
				$this->add_campaign_group();
				break;
			case 'add_campaign':
				$this->add_campaign();
				break;
			case 'save_airtable_mappings':
				$this->save_airtable_mappings();
				break;
			case 'add_airtable_mapping':
				$this->add_airtable_mapping();
				break;
			case 'sync_airtable':
				$this->sync_airtable();
				break;
	 ...
	 * @since 1.0.9
	 */
	private function add_country()
	{
		global $wpdb;

		$country_name_en = isset($_POST['country_name_en']) ? sanitize_text_field(wp_unslash($_POST['country_name_en'])) : '';
		$country_name_ar = isset($_POST['country_name_ar']) ? sanitize_text_field(wp_unslash($_POST['country_name_ar'])) : '';
		$country_code = isset($_POST['country_code']) ? strtoupper(sanitize_text_field(wp_unslash($_POST['country_code']))) : '';

		if (empty($country_name_en) || empty($country_name_ar) || empty($country_code)) {
			wp_die(esc_html__('All fields are required.', 'aqop-leads'), '', array('back_link' => true));
		}

		// Validate country code format (2 letters)
		if (!preg_match('/^[A-Z]{2}$/', $country_code)) {
			wp_die(esc_html__('Country code must be exactly 2 letters.', 'aqop-leads'), '', array('back_link' => true));
		}

		// Check if country code already exists
		$existing = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT id FROM {$wpdb->prefix}aq_dim_countries WHERE country_code = %s",
				$country_code
			)
		);

		if ($existing) {
			wp_die(esc_html__('Country code already exists.', 'aqop-leads'), '', array('back_link' => true));
		}
		...
		if ($inserted) {
			...
			wp_safe_redirect(
				add_query_arg(
					array(
						'page' => 'aqop-settings',
						'message' => 'country_added',
					),
					admin_url('admin.php')
				) . '#countries'
			);
		} else {
			wp_die(esc_html__('Failed to add country.', 'aqop-leads'), '', array('back_link' => true));
		}
		exit;
	}
	 ...
	 * @since 1.0.9
	 */
	private function add_campaign_group()
	{
		global $wpdb;

		$group_name_en = isset($_POST['group_name_en']) ? sanitize_text_field(wp_unslash($_POST['group_name_en'])) : '';
		$group_name_ar = isset($_POST['group_name_ar']) ? sanitize_text_field(wp_unslash($_POST['group_name_ar'])) : '';
		$group_description = isset($_POST['group_description']) ? sanitize_textarea_field(wp_unslash($_POST['group_description'])) : '';

		if (empty($group_name_en) || empty($group_name_ar)) {
			wp_die(esc_html__('Group name in both English and Arabic is required.', 'aqop-leads'), '', array('back_link' => true));
		}

		// Insert new campaign group
		$inserted = $wpdb->insert(
		...
		if ($inserted) {
			...
			wp_safe_redirect(
				add_query_arg(
					array(
						'page' => 'aqop-settings',
						'message' => 'campaign_group_added',
					),
					admin_url('admin.php')
				) . '#campaign-groups'
			);
		} else {
			wp_die(esc_html__('Failed to add campaign group.', 'aqop-leads'), '', array('back_link' => true));
		}
		exit;
	}
	...
	private function add_campaign()
	{
		...
		$campaign_group = isset($_POST['campaign_group']) ? intval($_POST['campaign_group']) : null;
		$campaign_platform = isset($_POST['campaign_platform']) && !empty($_POST['campaign_platform']) ? sanitize_text_field(wp_unslash($_POST['campaign_platform'])) : null;
		...
		$campaign_budget = isset($_POST['campaign_budget']) && !empty($_POST['campaign_budget']) ? floatval($_POST['campaign_budget']) : null;
		$campaign_description = isset($_POST['campaign_description']) ? sanitize_textarea_field(wp_unslash($_POST['campaign_description'])) : '';
		$campaign_status = isset($_POST['campaign_status']) ? 1 : 0;

		if (empty($campaign_name)) {
			wp_die(esc_html__('Campaign name is required.', 'aqop-leads'), '', array('back_link' => true));
		}

		// Validate dates if provided
		if ($start_date && $end_date && strtotime($start_date) > strtotime($end_date)) {
			wp_die(esc_html__('Start date cannot be after end date.', 'aqop-leads'), '', array('back_link' => true));
		}
		...
		$inserted = $wpdb->insert(
			...
			array(
				...
				'is_active' => $campaign_status,
				'created_at' => current_time('mysql'),
			),
			array('%d', '%s', '%s', '%s', '%s', '%s', '%f', '%d', '%s')
		);
		...
		if ($inserted) {
			...
			wp_safe_redirect(
				add_query_arg(
					array(
						'page' => 'aqop-settings',
						'message' => 'campaign_added',
					),
					admin_url('admin.php')
				) . '#campaigns'
			);
		} else {
			wp_die(esc_html__('Failed to add campaign.', 'aqop-leads'), '', array('back_link' => true));
		}
		exit;
	}

	/**
	 * Save Airtable field mappings.
	 *
	 * @since 1.0.9
	 */
	private function save_airtable_mappings()
	{
		...
		foreach ($mappings as $mapping) {
			if (!empty($mapping['airtable_field']) && !empty($mapping['wp_field'])) {
				$sanitized_mappings[] = array(
					'airtable_field' => sanitize_text_field($mapping['airtable_field']),
					'wp_field' => sanitize_text_field($mapping['wp_field']),
					'auto_create' => isset($mapping['auto_create']) ? (bool) $mapping['auto_create'] : false,
				);
			}
		}

		update_option('aqop_airtable_field_mapping', $sanitized_mappings);

		// Log event
		if (class_exists('AQOP_Event_Logger')) {
			AQOP_Event_Logger::log(
				'settings',
				'airtable_mappings_saved',
				'system',
				0,
				array(
					'mappings_count' => count($sanitized_mappings),
					'user_id' => get_current_user_id(),
				)
			);
		}
		...
		wp_safe_redirect(
			add_query_arg(
				array(
					'page' => 'aqop-settings',
					'message' => 'airtable_mappings_saved',
				),
				admin_url('admin.php')
			) . '#airtable-mapping'
		);
	...
	/**
	 * Add new Airtable field mapping.
	 *
	 * @since 1.0.9
	 */
	private function add_airtable_mapping()
	{
		$airtable_field = isset($_POST['airtable_field']) ? sanitize_text_field(wp_unslash($_POST['airtable_field'])) : '';
		$wp_field = isset($_POST['wp_field']) ? sanitize_text_field(wp_unslash($_POST['wp_field'])) : '';
		$auto_create = isset($_POST['auto_create']) ? (bool) $_POST['auto_create'] : false;

		if (empty($airtable_field) || empty($wp_field)) {
			wp_die(esc_html__('Airtable field name and WordPress field are both required.', 'aqop-leads'), '', array('back_link' => true));
		}

		// Get existing mappings
		$existing_mappings = get_option('aqop_airtable_field_mapping', array());

		// Fix: Handle if stored as JSON string instead of array
		if (is_string($existing_mappings) && !empty($existing_mappings)) {
			$decoded = json_decode($existing_mappings, true);
			if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
				$existing_mappings = $decoded;
			} else {
		...
		update_option('aqop_airtable_field_mapping', $existing_mappings);

		// Log event
		if (class_exists('AQOP_Event_Logger')) {
			AQOP_Event_Logger::log(
				'settings',
				'airtable_mapping_added',
				'system',
				0,
				array(
					'airtable_field' => $airtable_field,
		...
		wp_safe_redirect(
			add_query_arg(
				array(
					'page' => 'aqop-settings',
					'message' => 'airtable_mapping_added',
				),
				admin_url('admin.php')
			) . '#airtable-mapping'
		);
	...
	{
		// Check nonce
		if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'aqop_airtable_sync')) {
		...
		$interval = isset($_POST['interval']) ? sanitize_text_field(wp_unslash($_POST['interval'])) : 'every_30_minutes';
		$smart_sync = isset($_POST['smart_sync']) && $_POST['smart_sync'] === 'true';

		// Validate interval
		$valid_intervals = array('every_15_minutes', 'every_30_minutes', 'every_hour', 'every_6_hours', 'daily');
		if (!in_array($interval, $valid_intervals, true)) {
			$interval = 'every_30_minutes';
		}

		// Save the options
		update_option('aqop_airtable_auto_sync_enabled', $enabled);
		update_option('aqop_airtable_auto_sync_interval', $interval);
		update_option('aqop_airtable_smart_sync_enabled', $smart_sync);

		// If enabling, schedule the cron immediately
		if ($enabled) {
			...
			$next_run = wp_next_scheduled('aqop_airtable_auto_sync_hook');
			$next_run_formatted = $next_run ? date_i18n('M j, Y H:i:s', $next_run) : 'Not scheduled';
	...
	/**
	 * Sync from Airtable via AJAX
	 *
	 * @since 1.0.9
	 */
	public function sync_airtable()
	{
		// Enable error logging
		error_log('AQOP: Airtable sync started');

		try {
			// Check nonce
			if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'aqop_airtable_sync')) {
				error_log('AQOP: Invalid nonce in sync request');
				wp_send_json_error('Invalid nonce');
				return;
			}
			...
			// Check permissions
			if (!current_user_can('manage_options')) {
				error_log('AQOP: Insufficient permissions for sync');
				wp_send_json_error('Insufficient permissions');
				return;
			}

			// Check if sync class file exists
			$sync_file = AQOP_LEADS_PLUGIN_DIR . 'includes/class-airtable-sync.php';
			if (!file_exists($sync_file)) {
				error_log('AQOP: Sync class file not found: ' . $sync_file);
				wp_send_json_error('Sync class file not found');
				return;
			}

			// Require the sync class
			require_once $sync_file;

			// Check if class exists
			if (!class_exists('AQOP_Airtable_Sync')) {
				error_log('AQOP: AQOP_Airtable_Sync class not found after require_once');
				wp_send_json_error('Sync class not found');
				return;
			}

			error_log('AQOP: Starting sync execution');

			// Create sync instance and run sync
			$sync_instance = new AQOP_Airtable_Sync();
			$results = $sync_instance->sync_from_airtable();

			error_log('AQOP: Sync completed: ' . print_r($results, true));
			...
			if ($results['success']) {
				wp_send_json_success(array(
					...
					'stats' => array(
						...
						'countries_created' => $results['countries_created'],
						'campaigns_created' => $results['campaigns_created'],
						'groups_created' => $results['groups_created'],
						'sources_created' => $results['sources_created'],
					),
					'last_sync' => $sync_instance->get_last_sync_time(),
				));
			} else {
				error_log('AQOP: Sync failed with message: ' . $results['message']);
				wp_send_json_error($results['message']);
			}

		} catch (Exception $e) {
			error_log('AQOP: Sync exception: ' . $e->getMessage());
			error_log('AQOP: Stack trace: ' . $e->getTraceAsString());
			wp_send_json_error('Sync failed: ' . $e->getMessage());
		}
	}

	/**
	 * Test sync from Airtable (10 records) via AJAX
	 *
	 * @since 1.1.1
	 */
	public function test_sync_airtable()
	{
		error_log('AQOP: Test Airtable sync started');

		try {
			// Check nonce
			if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'aqop_airtable_sync')) {
				error_log('AQOP: Invalid nonce in test sync request');
				wp_send_json_error('Invalid nonce');
				return;
			}
			...
			// Check permissions
			if (!current_user_can('manage_options')) {
				error_log('AQOP: Insufficient permissions for test sync');
				wp_send_json_error('Insufficient permissions');
				return;
			}
			...
			// Check if class exists
			if (!class_exists('AQOP_Airtable_Sync')) {
				error_log('AQOP: AQOP_Airtable_Sync class not found');
				wp_send_json_error('Sync class not found');
				return;
			}

			error_log('AQOP: Starting test sync execution');

			// Create sync instance and run test sync
			$sync_instance = new AQOP_Airtable_Sync();
			$results = $sync_instance->test_sync_from_airtable();

			error_log('AQOP: Test sync completed: ' . print_r($results, true));
			...
			if ($results['success']) {
				wp_send_json_success(array(
					...
					'stats' => array(
						...
						'records_with_name' => $results['records_with_name'],
						'records_with_email' => $results['records_with_email'],
						'records_with_phone' => $results['records_with_phone'],
						'records_with_country' => $results['records_with_country'],
						'records_with_campaign' => $results['records_with_campaign'],
					),
					'sample_records' => $results['sample_records'],
				));
			} else {
				error_log('AQOP: Test sync failed with message: ' . $results['message']);
				wp_send_json_error($results['message']);
			}

		} catch (Exception $e) {
			error_log('AQOP: Test sync exception: ' . $e->getMessage());
			wp_send_json_error('Test sync failed: ' . $e->getMessage());
		}
	}

	/**
	 * Chunked sync from Airtable via AJAX (one batch at a time)
	 *
	 * @since 1.1.2
	 */
	public function sync_airtable_chunk()
	{
		error_log('AQOP: Airtable chunk sync started');

		try {
			// Check nonce
			if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'aqop_airtable_sync')) {
				error_log('AQOP: Invalid nonce in chunk sync request');
				wp_send_json_error('Invalid nonce');
				return;
			}
			...
			// Check permissions
			if (!current_user_can('manage_options')) {
				error_log('AQOP: Insufficient permissions for chunk sync');
				wp_send_json_error('Insufficient permissions');
				return;
			}

			// Get offset from request (empty for first chunk)
			$offset = isset($_POST['offset']) ? sanitize_text_field(wp_unslash($_POST['offset'])) : '';

			// Check if force full sync is requested (bypasses smart sync filter)
			$force_full_sync = isset($_POST['force_full_sync']) && ($_POST['force_full_sync'] === 'true' || $_POST['force_full_sync'] === true);

			// Check if sync class file exists
			$sync_file = AQOP_LEADS_PLUGIN_DIR . 'includes/class-airtable-sync.php';
			if (!file_exists($sync_file)) {
				error_log('AQOP: Sync class file not found');
				wp_send_json_error('Sync class file not found');
				return;
			}
			...
			if (!class_exists('AQOP_Airtable_Sync')) {
				error_log('AQOP: Sync class not found');
				wp_send_json_error('Sync class not found');
				return;
			}

			// Create sync instance and run chunk sync
			$sync_instance = new AQOP_Airtable_Sync();
			$results = $sync_instance->sync_chunk($offset, 50, $force_full_sync);

			error_log('AQOP: Chunk sync result: ' . print_r($results, true));
			...
			if ($results['success']) {
				wp_send_json_success(array(
					...
					'chunk_processed' => $results['chunk_processed'],
					'chunk_created' => $results['chunk_created'],
					'chunk_updated' => $results['chunk_updated'],
					'cumulative' => $results['cumulative'],
					'last_sync' => $results['is_complete'] ? $sync_instance->get_last_sync_time() : null,
				));
			} else {
				error_log('AQOP: Chunk sync failed: ' . $results['message']);
				wp_send_json_error($results['message']);
			}

		} catch (Exception $e) {
			error_log('AQOP: Chunk sync exception: ' . $e->getMessage());
			wp_send_json_error('Chunk sync failed: ' . $e->getMessage());
		}
	}

	/**
	 * Fetch Airtable fields via AJAX
	 *
	 * @since 1.0.9
	 */
	public function fetch_airtable_fields_ajax()
	{
		// Check nonce
		if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'aqop_fetch_airtable_fields')) {
			wp_send_json_error('Invalid nonce');
		}
		...
		try {
			...
			if (empty($base_id) || empty($table_name) || empty($api_key)) {
				wp_send_json_error('Base ID, table name, and API key are required');
			}
			...
			// Fetch fields
			$fields = AQOP_Airtable_Sync::fetch_airtable_fields($base_id, $table_name, $api_key);
	...
	private function render_leads_table()
	{
		...
		<form method="get" class="aqop-leads-filters">
			...
			<div class="search-box">
				...
				<input type="search" id="leads-search-input" name="s"
					...
					<tr>
						...
						<td>
							<span class="status-badge"
								style="background-color: <?php echo esc_attr($lead->status_color); ?>; color: white; padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: 600;">
	...
	{
		// Only on leads pages.
		if (strpos($hook, 'aqop-leads') === false && strpos($hook, 'aqop-settings') === false) {
			return;
		}
		...
		wp_localize_script(
			...
			array(
				...
				'strings' => array(
					'confirmBulkDelete' => __('Are you sure you want to delete %d leads?', 'aqop-leads'),
					...
					'testingConnection' => __('Testing connection...', 'aqop-leads'),
					'connectionSuccess' => __('Connection successful!', 'aqop-leads'),
					'connectionFailed' => __('Connection failed: ', 'aqop-leads'),
				),
			)
		);
	...
	{
		// Check nonce
		check_ajax_referer('aqop_campaign_questions', 'nonce');

		// Check permissions
		if (!current_user_can('manage_options')) {
			wp_send_json_error(
				array('message' => __('Permission denied.', 'aqop-leads')),
				403
			);
		}
		...
		$question_type = isset($_POST['question_type']) ? sanitize_text_field(wp_unslash($_POST['question_type'])) : 'text';

		// Validation
		if (empty($campaign_id) || empty($question_id) || empty($question_text)) {
			wp_send_json_error(
				array('message' => __('All fields are required.', 'aqop-leads')),
				400
			);
		}

		// Validate question ID format
		if (!preg_match('/^q[0-9]+$/', $question_id)) {
			wp_send_json_error(
				array('message' => __('Question ID must be in format: q1, q2, q3, etc.', 'aqop-leads')),
				400
			);
		}
		...
		if (!isset($all_questions[$campaign_id])) {
			$all_questions[$campaign_id] = array();
		}

		// Check if question ID already exists
		if (isset($all_questions[$campaign_id][$question_id])) {
			wp_send_json_error(
				array('message' => __('Question ID already exists for this campaign.', 'aqop-leads')),
				400
			);
		}

		// Add new question
		$all_questions[$campaign_id][$question_id] = array(
			'text' => $question_text,
			'type' => $question_type,
		);

		// Save to options
		update_option('aqop_campaign_questions', $all_questions);

		wp_send_json_success(
			array(
				'message' => __('Question added successfully.', 'aqop-leads'),
				'question' => $all_questions[$campaign_id][$question_id],
			)
		);
	}

	/**
	 * AJAX: Edit campaign question.
	 *
	 * @since 1.0.0
	 */
	public function ajax_edit_campaign_question()
	{
		...
		if (!isset($all_questions[$campaign_id][$question_id])) {
			wp_send_json_error(
				array('message' => __('Question not found.', 'aqop-leads')),
				404
			);
		}
		...
		wp_send_json_success(
			array(
				'message' => __('Question updated successfully.', 'aqop-leads'),
				'question' => $all_questions[$campaign_id][$question_id],
			)
		);
	}
	...
	public function ajax_delete_campaign_question()
	{
		...
		// Validation
		if (empty($campaign_id) || empty($question_id)) {
			wp_send_json_error(
				array('message' => __('Campaign ID and Question ID are required.', 'aqop-leads')),
				400
			);
		}
		...
		// Delete question
		unset($all_questions[$campaign_id][$question_id]);

		// If no questions left for campaign, remove campaign key
		if (empty($all_questions[$campaign_id]) && isset($all_questions[$campaign_id])) {
			unset($all_questions[$campaign_id]);
		}
		...
		wp_send_json_success(
			array('message' => __('Question deleted successfully.', 'aqop-leads'))
		);
	}
	 ...
	 * @since 1.0.0
	 */
	private function update_cors_settings()
	{
		$jwt_allowed_origins = isset($_POST['jwt_allowed_origins']) ? sanitize_textarea_field(wp_unslash($_POST['jwt_allowed_origins'])) : '';

		// Update option
		update_option('aqop_jwt_allowed_origins', $jwt_allowed_origins);
	...
	/**
	 * Check if user has AQOP access.
	 *
	 * @since 1.0.0
	 * @return bool True if user has any AQOP role.
	 */
	private function user_has_aqop_access()
	{
		$user = wp_get_current_user();
		$aqop_roles = array('administrator', 'operation_admin', 'operation_manager', 'aq_supervisor', 'aq_agent');
	 ...
	 * @since 1.0.0
	 */
	private function update_meta_integration_settings()
	{
		$verify_token = isset($_POST['meta_verify_token']) ? sanitize_text_field(wp_unslash($_POST['meta_verify_token'])) : '';
		$app_secret = isset($_POST['meta_app_secret']) ? sanitize_text_field(wp_unslash($_POST['meta_app_secret'])) : '';

		update_option('aqop_meta_verify_token', $verify_token);
		update_option('aqop_meta_app_secret', $app_secret);

		wp_safe_redirect(
			add_query_arg(
				array(
					'page' => 'aqop-settings',
					'message' => 'meta_settings_saved',
				),
				admin_url('admin.php')
			) . '#meta-integration'
		);
	...
	private function update_whatsapp_settings()
	{
		$phone_number_id = isset($_POST['aqop_whatsapp_phone_number_id']) ? sanitize_text_field(wp_unslash($_POST['aqop_whatsapp_phone_number_id'])) : '';
		$business_account_id = isset($_POST['aqop_whatsapp_business_account_id']) ? sanitize_text_field(wp_unslash($_POST['aqop_whatsapp_business_account_id'])) : '';
		$access_token = isset($_POST['aqop_whatsapp_access_token']) ? sanitize_text_field(wp_unslash($_POST['aqop_whatsapp_access_token'])) : '';
		$enabled = isset($_POST['aqop_whatsapp_enabled']) ? '1' : '0';

		update_option('aqop_whatsapp_phone_number_id', $phone_number_id);
		update_option('aqop_whatsapp_business_account_id', $business_account_id);
		update_option('aqop_whatsapp_access_token', $access_token);
		update_option('aqop_whatsapp_enabled', $enabled);
	...
	/**
	 * Get Meta test payload via AJAX
	 *
	 * @since 1.0.0
	 */
	public function ajax_get_meta_test_payload()
	{
		check_ajax_referer('aqop_meta_test', 'nonce');
		...
		$test_data = AQOP_Meta_Webhook_API::test_webhook();
	...
	public function ajax_test_whatsapp_connection()
	{
		...
		if (!current_user_can('manage_options')) {
			wp_send_json_error(array('message' => __('Permission denied.', 'aqop-leads')));
		}

		$phone_number_id = get_option('aqop_whatsapp_phone_number_id');
		$access_token = get_option('aqop_whatsapp_access_token');

		if (empty($phone_number_id) || empty($access_token)) {
			wp_send_json_error(array('message' => __('Phone Number ID and Access Token are required.', 'aqop-leads')));
		}
		...
		if (isset($data['id']) && $data['id'] === $phone_number_id) {
			wp_send_json_success(array('message' => __('Connection successful!', 'aqop-leads')));
		} else {
			wp_send_json_error(array('message' => __('Unexpected response from WhatsApp API.', 'aqop-leads')));
		}
	}
}
