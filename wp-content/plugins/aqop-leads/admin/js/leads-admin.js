...
(function ($) {
	...
	const AQOP_Leads = {
		...
		// === WHATSAPP SETTINGS (Phase 4.1) ===

		/**
		 * Handle WhatsApp connection test
		 */
		handleWhatsAppTest: function () {
			$('#test-whatsapp-connection').on('click', function (e) {
				e.preventDefault();

				var $button = $(this);
				var $result = $('#whatsapp-test-result');
				var originalText = $button.text();
				...
				$button.prop('disabled', true).html(
					'<span class="dashicons dashicons-update spin"></span> ' +
					(aqopLeads.strings.testingConnection || 'Testing connection...')
				);
				...
				$.ajax({
					...
					method: 'POST',
					data: {
						action: 'aqop_test_whatsapp_connection',
						nonce: aqopLeads.nonce
					},
					success: function (response) {
						if (response.success) {
							$result.html(
								...
								(response.data.message || aqopLeads.strings.connectionSuccess) +
								'</span>'
							);
						} else {
							$result.html(
								...
								(aqopLeads.strings.connectionFailed || 'Connection failed: ') +
								(response.data.message || aqopLeads.strings.error) +
								'</span>'
							);
						}
					},
					error: function () {
						$result.html(
							'<span style="color: #dc3232; font-weight: bold;">' +
							'<span class="dashicons dashicons-no"></span> ' +
							(aqopLeads.strings.error || 'An error occurred.') +
							'</span>'
						);
					},
					complete: function () {
