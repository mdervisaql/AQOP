...
(function ($) {
	...
	var LeadDetail = {
		...
		init: function () {
			...
			this.handleScoreRecalculation();
		},

		/**
		 * Handle score recalculation
		 */
		handleScoreRecalculation: function () {
			$('#recalculate-score').on('click', function () {
				...
				$button.prop('disabled', true).html(
					...
					(aqopLeads.strings.recalculating || 'Recalculating...')
				);

				$.ajax({
					url: aqopLeads.root + 'aqop/v1/leads/' + leadId + '/recalculate-score',
					method: 'POST',
					beforeSend: function (xhr) {
						xhr.setRequestHeader('X-WP-Nonce', aqopLeads.nonce);
					},
					success: function (response) {
						if (response.success) {
							LeadDetail.showNotice('success', 'Score recalculated successfully.');
							...
							LeadDetail.showNotice('error', 'Failed to recalculate score.');
